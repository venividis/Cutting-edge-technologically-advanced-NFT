...
const abi = [
  ...
  { type:'function', name:'setManifest', stateMutability:'nonpayable', inputs:[{name:'agentId',type:'uint256'},{name:'agentURI',type:'string'},{name:'manifestHash',type:'bytes32'}], outputs:[] },
...
// The metadata is stored with the token instead of pointing at this website. Wallets such as
// Brave can therefore render a freshly minted ANIMA without an IPFS gateway or hosted API.
function metadataURI(name,hue,agentId,model){
  const escaped=safe(name);
  const image=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000"><defs><radialGradient id="g"><stop stop-color="hsl(${hue} 90% 70%)"/><stop offset="1" stop-color="#09090b"/></radialGradient></defs><rect width="1000" height="1000" fill="#09090b"/><circle cx="500" cy="455" r="330" fill="url(#g)" opacity=".45"/><g fill="none" stroke="hsl(${hue} 85% 78%)" stroke-width="4"><ellipse cx="500" cy="455" rx="220" ry="350"/><ellipse cx="500" cy="455" rx="220" ry="350" transform="rotate(60 500 455)"/><ellipse cx="500" cy="455" rx="220" ry="350" transform="rotate(120 500 455)"/><circle cx="500" cy="455" r="175"/><path d="M500 105 810 630 190 630Z"/></g><text x="500" y="875" text-anchor="middle" fill="#eee9df" font-family="serif" font-size="70">${escaped}</text><text x="500" y="925" text-anchor="middle" fill="#d9bd77" font-family="monospace" font-size="22">ANIMA · SOVEREIGN AGENT</text></svg>`;
  const imageURI=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(image)}`;
  const registry=`eip155:84532:${ANIMA}`;
  const manifest={type:'https://eips.ethereum.org/EIPS/eip-8004#registration-v1',name:`ANIMA — ${name}`,description:'A sovereign, economically accountable AI agent with a deterministic ERC-6551 web3 account.',image:imageURI,services:[],x402Support:false,active:true,registrations:[{agentId:Number(agentId),agentRegistry:registry}],anima:{registry,agentId:String(agentId),model:{modelId:model.modelId,weightsRoot:model.weightsRoot,attestationKind:model.attestationKind}}};
  return `data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify(manifest))}`;
}

async function addNFTToWallet(agent){
  if(!window.ethereum)return toast('Open this page in Brave Wallet or another Web3 wallet.');
  try{
    const added=await window.ethereum.request({method:'wallet_watchAsset',params:{type:'ERC721',options:{address:ANIMA,tokenId:`0x${agent.id.toString(16)}`}}});
    toast(added?'NFT added to your wallet.':'The wallet did not add the NFT.');
  }catch(e){toast(e?.message?.includes('not supported')?'This wallet cannot add NFTs automatically. Use the contract and token ID shown here.':(e.shortMessage||e.message||'Add-to-wallet request was cancelled.'))}
}

async function copyAccount(agent){
  try{await navigator.clipboard.writeText(agent.account);toast('Web3 account address copied.')}catch{toast(`Account: ${agent.account}`)}
}
...
function showConsole(a){selectedAgent=a;const el=document.querySelector('.agent-console');el.hidden=false;el.innerHTML=`<div class="console-title"><div><span class="section-no">COMMAND CHAMBER · AGENT ${a.id}</span><h3>${statuses[a.status]} <i class="life s${a.status}"></i></h3></div><p>Every command is simulated before your wallet is asked to sign.</p></div><div class="console-metrics"><div><span>STATE FINGERPRINT</span><b>${short(a.fingerprint,12)}</b></div><div><span>MEMORY</span><b>${seals[a.seal]} / EPOCH ${a.epoch}</b></div><div><span>WEB3 ACCOUNT</span><b title="${a.account}">${short(a.account,9)}</b><small>${a.deployed?'◆ LIVE ERC-6551 ADDRESS':'◇ ADDRESS RESERVED'}</small></div><div><span>DAILY HORIZON</span><b>${Number(formatEther(a.policy.dailyWei)).toFixed(4)} ETH</b></div></div><div class="web3-address"><span>${a.account}</span><button data-copy>Copy address</button></div><div class="console-actions"><button data-action="status" data-value="${a.status===1?2:1}">${a.status===1?'Pause safely':'Awaken agent'}</button>${a.deployed?'':'<button data-action="deploy">Activate web3 address</button>'}<button data-wallet>Add NFT to Brave</button><a target="_blank" rel="noreferrer" href="${explorer}/token/${ANIMA}?a=${a.id}">Open provenance ↗</a></div><p class="safety-note">◇ BRAVE IMPORT: BASE SEPOLIA · CONTRACT ${ANIMA} · TOKEN ID ${a.id} · NO PRIVATE KEYS ENTER THIS PAGE</p>`;el.scrollIntoView({behavior:'smooth'});el.querySelectorAll('[data-action]').forEach(b=>b.onclick=()=>transact(b));el.querySelector('[data-wallet]').onclick=()=>addNFTToWallet(a);el.querySelector('[data-copy]').onclick=()=>copyAccount(a)}
...
async function connect(){if(!window.ethereum)return toast('No browser wallet found. Open this page in Brave and enable Brave Wallet.');const b=document.querySelector('.connect');b.textContent='Opening wallet…';try{walletClient=createWalletClient({chain:baseSepolia,transport:custom(window.ethereum)});[walletAddress]=await walletClient.requestAddresses();if(await walletClient.getChainId()!==baseSepolia.id){try{await walletClient.switchChain({id:baseSepolia.id})}catch{await window.ethereum.request({method:'wallet_addEthereumChain',params:[{chainId:'0x14a34',chainName:'Base Sepolia',nativeCurrency:{name:'Ether',symbol:'ETH',decimals:18},rpcUrls:['https://sepolia.base.org'],blockExplorerUrls:[explorer]}]})}}await discover()}catch(e){toast(e.shortMessage||e.message||'Connection cancelled.')}finally{b.innerHTML='Connect wallet <span>↗</span>'}}
...
async function mintAgent(){if(!walletClient||!walletAddress){await connect();if(!walletClient||!walletAddress)return}const name=(window.prompt('Name your testnet agent','Astra')||'').trim();if(!name)return;const seed=`ANIMA:${walletAddress}:${name}:${Date.now()}`;const model={weightsRoot:keccak256(toHex(`${seed}:model`)),runtimeMeasurement:zeroHash,attestationKind:0,modelId:`anima/${name.toLowerCase().replace(/[^a-z0-9-]/g,'-')}`};const shards=[{dataHash:keccak256(toHex(`${seed}:memory`)),keyCommitment:zeroHash,size:BigInt(new TextEncoder().encode(seed).length),kind:1,uri:'',description:'Genesis memory commitment'}];let agent;try{toast('Simulating your agent’s birth…');const {request}=await chainClient.simulateContract({address:ANIMA,abi,functionName:'mintAgent',args:[walletAddress,'',zeroHash,model,shards,0,[]],account:walletAddress});const hash=await walletClient.writeContract(request);toast('Birth submitted. Waiting for Base Sepolia…');const receipt=await chainClient.waitForTransactionReceipt({hash});const birth=parseEventLogs({abi,eventName:'Transfer',logs:receipt.logs}).find(log=>log.args.from===zeroAddress);if(!birth)throw new Error('Mint receipt did not contain the new token ID.');agent=await loadAgent(birth.args.tokenId)}catch(e){return toast(e.shortMessage||e.message||'Mint cancelled or refused.')}try{const hue=Number((agent.id*47n+32n)%360n);const uri=metadataURI(name,hue,agent.id,model);toast(`Agent ${agent.id} minted. Confirm its ID-bound registration.`);const {request}=await chainClient.simulateContract({address:ANIMA,abi,functionName:'setManifest',args:[agent.id,uri,keccak256(toHex(uri))],account:walletAddress});const hash=await walletClient.writeContract(request);await chainClient.waitForTransactionReceipt({hash})}catch(e){toast(`NFT minted, but its registration still needs to be confirmed.`);await discover();showConsole(agent);return}try{toast(`Registration committed. Approve once more to activate its web3 address.`);const {request}=await chainClient.simulateContract({address:ANIMA,abi,functionName:'deployAccount',args:[agent.id],account:walletAddress});const hash=await walletClient.writeContract(request);await chainClient.waitForTransactionReceipt({hash});agent=await loadAgent(agent.id);toast(`Web3 address ${short(agent.account,8)} is live.`)}catch(e){toast(`NFT minted. Its reserved address can be activated later.`)}await discover();showConsole(agent);await addNFTToWallet(agent)}
