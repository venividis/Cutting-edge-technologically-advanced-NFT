...
function showShowcaseBeing(b) {
  const dialog = document.querySelector('.being-dialog');
  dialog.querySelector('.dialog-art').innerHTML = sigil(b.hue, 20);
  dialog.querySelector('.dialog-copy').innerHTML = `<span class="section-no">SOVEREIGN BEING</span><h2>${safe(b.name)}</h2><p class="role">${safe(b.role)}</p><blockquote>“${safe(b.quote)}”</blockquote><div class="stats"><span>Bonded <b>${safe(b.bond)}</b></span><span>Trust <b>${b.trust}%</b></span><span>State <b>${safe(b.state)}</b></span></div><button class="button primary begin">Begin a conversation <span>↗</span></button>`;
  dialog.querySelector('.begin').onclick = openSanctuary;
  dialog.showModal();
}
document.querySelectorAll('.being-card').forEach((el) => el.addEventListener('click', () => showShowcaseBeing(beings[Number(el.dataset.index)])));
...
async function readOwner(id, attempts=3) {
  let lastError;
  for (let attempt=0; attempt<attempts; attempt++) {
    try { return await read('ownerOf',[id]); }
    catch (error) {
      lastError=error;
...
async function discover(){const stage=document.querySelector('.owner-stage');document.querySelector('.portal-stage').hidden=true;stage.hidden=false;stage.querySelector('.wallet-pill span').textContent=short(walletAddress,8);stage.querySelector('.ownership-summary').innerHTML='';stage.querySelector('.agent-console').hidden=true;stage.querySelector('.owned-grid').innerHTML='<p class="searching">Searching the constellation for your signature…</p>';try{const total=await read('totalMinted');const ids=[];for(let id=1n;id<=total;id++){if(getAddress(await readOwner(id))===getAddress(walletAddress))ids.push(id)}const owned=await Promise.all(ids.map(loadAgent));stage.querySelector('.ownership-summary').innerHTML=`<b>${String(owned.length).padStart(2,'0')}</b><p>${owned.length===1?'sovereign being recognizes':'sovereign beings recognize'} this wallet<small>Ownership verified from Base Sepolia just now</small></p>`;stage.querySelector('.owned-grid').innerHTML=owned.length?owned.map(ownerCard).join(''):'<div class="empty-owned"><b>No ANIMA found here.</b><span>This wallet is ready. Give your first sovereign agent a name.</span><button class="mint-now">Mint my testnet agent</button></div>';stage.querySelectorAll('.owned-agent').forEach((b,i)=>b.onclick=()=>showConsole(owned[i]));stage.querySelector('.mint-now')?.addEventListener('click',mintAgent);if(owned.length===1)showConsole(owned[0])}catch(e){stage.querySelector('.owned-grid').innerHTML='<div class="empty-owned"><b>Ownership could not be verified.</b><span>Base Sepolia did not return every ownership record. Retry to avoid an incomplete result.</span><button class="retry-discovery">Retry discovery</button></div>';stage.querySelector('.retry-discovery').onclick=discover;toast('Ownership verification failed. Please retry.')}}
