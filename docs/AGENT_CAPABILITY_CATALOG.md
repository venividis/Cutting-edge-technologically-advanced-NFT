...
An ERC-8004 registration may list content-addressed `anima.extensions`. The SDK validates the
MASTER-compatible `anima.extension-release/1` format, its five explicit host permissions, archive
resource bounds, canonical SHA-256 identity and a finite acyclic dependency graph. Declaring or
installing an extension does not grant it wallet authority; transaction and journal capabilities
mean “may propose”, not “may sign”. The current Base Sepolia deployment has no module registry, so
these declarations are discovery commitments rather than claims of onchain installation.
